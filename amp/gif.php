...
if (isset($_GET['url'])) {
    ...
    if ($gif_up_cat==0) {
        $sql_gif = 'SELECT g.id, g.url, g.dt_add, category_id, u.name, title, question, likes_count, favs_count, views_count, points, avg_points, votes, c.nameCat, c.urlCat FROM gifs g JOIN categories c ON g.category_id = c.id JOIN users u ON g.user_id = u.id  WHERE g.url = "'.$gif_url.'"';
    } else {
        $sql_gif = 'SELECT g.id, g.url, g.dt_add,category_id, u.name, title, question, likes_count, favs_count, views_count, points, avg_points, votes, c.nameCat, c.urlCat, up.up_id, up.name_up_Cat, up.url_up_Cat FROM gifs g JOIN categories c ON g.category_id = c.id JOIN upcategories up ON up.up_id = c.upcategories_id JOIN users u ON g.user_id = u.id WHERE g.url = "'.$gif_url.'"';
    }
...
// узнаем категорию, чтоб по категории определить нет ли в апкатегории 0
    ...
    if ($gif_up_cat==0) {
        $sql_gif = 'SELECT g.id, g.url, g.dt_add,category_id, u.name, title, question, likes_count, favs_count, views_count, points, avg_points, votes, c.nameCat, c.urlCat FROM gifs g '.'JOIN categories c ON g.category_id = c.id '.'JOIN users u ON g.user_id = u.id '.'WHERE g.id = '.$gif_id;
    } else {
...
if (isset($_SESSION['user'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sql_gif = 'SELECT g.id, category_id, u.name, title, question, likes_count, favs_count, views_count, points, avg_points, votes, g.url, c.nameCat, c.urlCat FROM gifs g JOIN categories c ON g.category_id = c.id JOIN users u ON g.user_id = u.id WHERE g.id = '.$gif_id;
        $res_gif = mysqli_query($connect, $sql_gif);
        if ($res_gif) {
            $gif = mysqli_fetch_assoc($res_gif);
        } else {
...
if ($is404error) {
    ...
    if (isset($_SESSION['user'])) {
        $page_content = include_template('gif-amp.php', ['username' => $_SESSION['user']['name'], 'errors' => $errors, 'gif' => $gif, 'count_comm' => $count_comm,  'comments' => $comments, 'gifs' => $similar_gifs, 'gif_id' => $gif_id, 'isGifPage' => $isGifPage, 'isFav' => $isFav, 'isLiked' => $isLiked, 'dict' => $dict]);
        $layout_content = include_template('layout-amp.php', ['username' => $_SESSION['user']['name'], 'content' => $page_content, 'href' => $href, 'href_amp' => $href_amp, 'gif' => $gif, 'Js' => $Js, 'categories' => $categories, 'upcategories' => $upcategories, 'num_online' => $num_online, 'num_visitors_hosts' => $row[0]['hosts'], 'num_visitors_views' => $row[0]['views'], 'hosts_stat_month' => $hosts_stat_month, 'views_stat_month' => $views_stat_month, 'title' => $gif['title']]);
    } else {
        $layout_content = include_template('layout-amp.php', ['content' => $page_content, 'upcategories' => $upcategories, 'categories' => $categories, 'href_amp' => $href_amp, 'gif' => $gif, 'href' => $href, 'num_online' => $num_online, 'Js' => $Js, 'num_visitors_hosts' => $row[0]['hosts'], 'num_visitors_views' => $row[0]['views'], 'hosts_stat_month' => $hosts_stat_month, 'views_stat_month' => $views_stat_month, 'title' => $gif['title']]);
    }
}
