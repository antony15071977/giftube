...
if ($res_token) {
	...
	if (!empty($token_row)) {
		...
		if ($token == $token_in) {
			...
			if (!$res_update_user) {
				$error = mysqli_error($connect);
				$info = '<p><strong>Ошибка!</strong>Сбой при обновлении статуса пользователя. Код ошибки: '.$error.
				'</p>';
			} else {
				...
				if (!$res_sql_del_conf_user) {
					...
					$error = mysqli_error($connect);
					$info = '<p><strong>Ошибка!</strong>Сбой при удалении данных пользователя из временной таблицы. Код ошибки: '.$error.
					'</p>';
				} else {
	...
	$error = mysqli_error($connect);
	$info = '<p><strong>Ошибка!</strong> Сбой при выборе пользователя из БД. Код ошибки: '.$error.
	'</p>';
}
