...
if (!isset($_SESSION['user'])) {
				...
				if ($user_password == $user[0]['password']) {
					...
					if ($email_status == 0) {
						...
						if (isset($_POST["remember"])) {
							//Создаём токен
							$cookie_token = md5($user[0]['secretkey'].
								":".$_SERVER["REMOTE_ADDR"].
								":".$user[0]['dt_add']);
	...
	$layout_content = include_template('layout.php', ['username' => $_SESSION['user']['name'], 'content' => $page_content, 'categories' => $categories, 'num_online' => $num_online, 'num_visitors_hosts' => $row[0]['hosts'], 'num_visitors_views' => $row[0]['views'], 'hosts_stat_month' => $hosts_stat_month, 'views_stat_month' => $views_stat_month, 'Js' => $Js, 'title' => 'Вход на сайт']);
