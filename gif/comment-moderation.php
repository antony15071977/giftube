...
if (isset($_SESSION['user'])&&$_SESSION['user']['status']==3) {
    if (isset($_GET["ok"])) {
        ...
        $_SESSION["success_messages"] = "Комментарий одобрен и опубликован.";
    }
    if (isset($_GET["del"])) {
        $com_id = htmlspecialchars(intval($_GET['del']));
        $res=mysqli_query($connect,"DELETE FROM comments WHERE id='".$com_id."'");
        $_SESSION["success_messages"] = "Комментарий удален.";
    }
    if ($_GET["comment"]) {
    ...
    if ($_POST["comment"]!='') {
     ...
     SET comment_text='".htmlspecialchars($_POST["comment"])."', moderation=1 WHERE id='".$com_id."'");
       $_SESSION["success_messages"] = "Комментарий исправлен, одобрен и опубликован.";
    }
} else {
    ...
    $layout_content = include_template('layout.php', ['username' => $_SESSION['user']['name'], 'content' => $page_content, 'Js' => $Js, 'categories' => $categories, 'num_online' => $num_online, 'num_visitors_hosts' => $row[0]['hosts'], 'num_visitors_views' => $row[0]['views'], 'hosts_stat_month' => $hosts_stat_month, 'views_stat_month' => $views_stat_month]);
    print($layout_content);
    exit();
}
$page_content = include_template('comment-moderation.php', ['com' => $com]);
$layout_content = include_template('layout.php', ['username' => $_SESSION['user']['name'], 'content' => $page_content,  'categories' => $categories, 'num_online' => $num_online, 'num_visitors_hosts' => $row[0]['hosts'], 'num_visitors_views' => $row[0]['views'], 'hosts_stat_month' => $hosts_stat_month, 'views_stat_month' => $views_stat_month]);
print($layout_content);